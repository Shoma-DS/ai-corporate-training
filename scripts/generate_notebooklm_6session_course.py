...
BASE_SLIDES = [
    ("{title}", "第{no}回の到達点と成果物を確認する", "module-block"),
    ("この回で作る成果物", "{output}", "checklist-confirmation"),
    ("業務課題から始める理由", "機能紹介ではなく、確認・説明・共有の業務改善として扱う", "before-after-transformation"),
    ("6回ロードマップ上の位置づけ", "前回成果物との接続と次回への受け渡しを確認する", "process-flow"),
    ("情報管理の前提", "実データ、個人情報、契約情報、未公開資料を演習に使わない", "governance-risk"),
    ("今回の業務ケース", "{case}", "module-block"),
    ("Before: 現場で起きている詰まり", "資料分散、最新版不明、根拠確認漏れ、担当者依存", "before-after-transformation"),
    ("After: NotebookLMを業務に組み込む姿", "情報源、質問、確認、共有、更新を一連の流れにする", "process-flow"),
    ("画面共有: 講師記入例を見る", "この回で使うサンプル資料とワークシートを確認する", "scene-environment"),
    ("作業の全体像", "入力、処理、出力、確認、保存、共有、改善の流れ", "process-flow"),
    ("判断基準1: NotebookLMに向く業務", "複数資料を横断し、根拠付きで確認したい業務", "matrix-classification"),
    ("判断基準2: 任せない業務", "承認、例外判断、社外送信、機密情報を含む判断", "governance-risk"),
    ("ワーク1: 現状を書き出す", "動画を一時停止して、課題と資料をワークシートへ記入する", "checklist-confirmation"),
    ("講師記入例の確認", "記入粒度、抽象化、公開不可情報の除外を確認する", "data-insight"),
    ("自己レビュー1", "業務課題、資料、困りごと、成果物がつながっているか", "checklist-confirmation"),
    ("資料を情報源として設計する", "資料名、形式、目的、更新日、責任者、機密区分を揃える", "data-insight"),
    ("質問は目的から逆算する", "読み手、出力形式、根拠確認方法をセットで設計する", "process-flow"),
    ("根拠付き回答の見方", "引用、根拠資料、未確認事項、追加確認先を分ける", "checklist-confirmation"),
    ("画面共有: NotebookLMで質問する", "ダミー資料を使って根拠付き回答の作り方を見る", "scene-environment"),
    ("良い質問と悪い質問", "曖昧な依頼を、検証可能な質問へ直す", "comparison-contrast"),
    ("出力を業務文書へ変換する", "FAQ、説明文、アクション一覧、提案メモに整える", "process-flow"),
    ("人が確認する位置を残す", "AI下書き、原典確認、担当者レビュー、最終判断を分ける", "governance-risk"),
    ("ワーク2: 根拠付き出力を作る", "動画を一時停止して、指定の成果物を作成する", "checklist-confirmation"),
    ("講師記入例との比較", "根拠、未確認、追加確認が分かれているか確認する", "data-insight"),
    ("よくある失敗", "古い資料、過剰な断定、根拠なし回答、共有範囲ミス", "governance-risk"),
    ("Studio出力の使い分け", "ブリーフィング、Mind Map、Audio Overview等を目的別に扱う", "module-block"),
    ("共有前チェック", "誰に見せてよいか、何を削るか、誰が承認するか", "governance-risk"),
    ("更新ルールを決める", "更新担当、更新頻度、差し替え、削除、停止条件", "structure-layer"),
    ("画面共有: 運用ルールの記入例", "運用設計シートを講師例で確認する", "scene-environment"),
    ("リスクを一覧化する", "情報漏洩、著作権、ハルシネーション、権限過多", "governance-risk"),
    ("効果を説明できる形にする", "探索時間、一次回答作成、会議後整理、品質安定", "data-insight"),
    ("小さく始める導入ステップ", "1業務、1部署、1ノートブック、1か月で試す", "process-flow"),
    ("ワーク3: 活用設計を完成する", "動画を一時停止して、対象業務と運用ルールを記入する", "checklist-confirmation"),
    ("講師記入例の最終確認", "成果物の抜け漏れと安全性を確認する", "checklist-confirmation"),
    ("自部署へ置き換える観点", "資料、役割、承認、更新、KPIを自部署名なしで整理する", "module-block"),
    ("ワーク4: 次アクションを決める", "最初に試すテーマ、確認先、期限、レビュー方法を書く", "checklist-confirmation"),
    ("第{no}回の成果物チェック", "{output}", "checklist-confirmation"),
    ("次回への受け渡し", "今回の成果物を次回の設計・実演・提案へ接続する", "process-flow"),
    ("受講後の自己点検", "公開不可情報、根拠確認、人のレビュー、更新ルール", "governance-risk"),
    ("まとめ", "{capability}", "module-block"),
]
