...
- For every non-GAS course session, use the canonical GAS course `講座/生成AI・GASで実践する業務変革・DX推進講座/` as the baseline for design, composition, image-prompt specificity, pamphlet consistency, and script feel. Match its white-background navy/teal card hierarchy, conclusion-style top sentence, structured blocks, table/process/checklist/canvas use, bottom exercise/output/risk band, and mouse-pointable video narration. Keep the target course's own theme, tools, risks, datasets, outputs, and examples; never copy GAS-specific content.
...
- If `imagegen` or the local responses endpoint returns `502 Bad Gateway`, response-not-complete, WebSocket close, or a similar transport error, do not stop the session build. Check the current worker's `$CODEX_HOME/generated_images/<session-id>/` for delayed output, and use `scripts/copy_latest_generated_image.py --wait-seconds 120 --missing-ok --status-json ...` when copying. If no bitmap appears after a short wait/retry, record that Sxx as pending and continue with the next slide. Do not fall back to API keys, the OpenAI CLI, one-off SDK scripts, or local placeholder images.
...
- Run lightweight checks on every generated image: missing files, valid bitmap/MIME, dimensions/aspect, abnormal file size, duplicate hashes, generation-log pending/error states, known stale course names, and forbidden terms by OCR/search where available.
- Visually inspect only images flagged by lightweight checks, images with suspicious OCR, images using official logos/screenshots/new UI material, or images the user specifically asks to inspect.
...
- Add a clear `目次/全体像` slide near the beginning of every session and `章見出し/現在位置` slides at the start of major sections. These are required for future courses, not only the GAS course. They must show the current agenda item, what comes before/after, upcoming decision points, and the learner output connected to that section.
...
- Generate `図解パーツ生成プロンプト.md` for dense supplemental visuals/reference-sheet images only. It must explicitly say not to render course headers, Sxx numbers, session names, speaker notes, or long paragraph text inside the image. It should also say that core comparison tables, process tables, decision canvases, and industry-example cells may be rendered when they are the source slide's main information.
- Generate and inspect one supplemental PNG per required slide as `図解パーツ/Sxx.png`. The image should normally contain readable Japanese headings, table cells, card text, process names, output names, decision axes, learner actions, concrete examples, or review/risk notes so it functions as supplemental material; decorative text-free icons or short-label-only visuals are not enough. For S04-like slides, the image should preserve multiple structures in one raster image, such as Before/After table + hierarchy + industry examples. It must not contain fake UI/logos, stale course names, recruitment-ad/poster layouts, placeholders, or long unreadable text dumps. Do not mark a slide as generated if the image cannot be saved locally and inspected.
- Diagram/reference images should be wide and slightly vertically compact when used inside an editable template. Leave visual room for editable title, Sxx number, section name, and current-position labels.
- Re-export editable Google Slides with `--embed-diagram-parts` and `--make-diagram-images-readable-by-link` only after the expected `図解パーツ/Sxx.png` files exist. The embedded diagrams must supplement the native editable text, not replace it or cover it. Text-only template export requires `--allow-text-only-template-export` and is only for temporary debugging.
...
- For editable Google Slides diagram parts, run `python3 scripts/check_diagram_parts.py --course-dir '講座/COURSE'` to catch missing PNGs, suspicious dimensions, and duplicate hashes caused by copying another worker's generated image. The older `scripts/check_diagram_integrity.py` is only for the fixed Google Workspace/GAS course.
...
- Confirm lightweight checks were run for all generated images, and record only the S番号 escalated to visual inspection plus the reason.
