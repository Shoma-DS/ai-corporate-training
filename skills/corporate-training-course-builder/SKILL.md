...
- Business-expansion reskilling helper: `skills/business-expansion-reskilling-builder/SKILL.md` is used only for courses under `事業展開等リスキリング講座/`. It keeps the official job-directness and LMS gates while replacing this skill's dense screening-slide/image rules with the nearer subtree's lecture-first editable HTML/PDF standard.
- Training idea helper: `skills/training-idea-project-manager/SKILL.md` captures public-safe ideas and private Project drafts after a concept is identified. It classifies the idea by subsidy context and never replaces course production.
...
For `講座/AX・DXワークショップ講座/`, always consult the accepted reference PDFs `AX・DXワークショップ講座_第1回 .pdf` and `AX・DXワークショップ講座_第2回.pdf` when creating or revising slide plans, scripts, image prompts, or slide images. Use them as per-slide density benchmarks only: So What headlines, 3-6 concrete content blocks, comparison tables, process diagrams, case-study specificity, risk-example separation, and summary style. Do not copy the PDFs' prose into public files and do not store the source PDFs in the public repository. Course structure and total slide count may vary by course/session intent; do not force exact counts. The acceptance criterion is that each individual slide has comparable information density and concrete explanatory value to the reference PDFs.
...
- Course folder names should follow the latest public-facing training name unless a user explicitly asks to preserve a legacy path for compatibility. Treat old file prefixes and historical names as legacy references, not as the visible course title.
...
For `講座/生成AI・GASで実践する業務変革・DX推進講座/`, the public-facing training name and canonical course folder name are both `生成AI・GASで実践する業務変革・DX推進講座`. The older `Google Workspace・GASで進めるAI業務効率化-DX実践講座` name is a legacy mismatch and must not be reused as the repository path, Drive folder name, or visible training title.
...
Exception: courses under `事業展開等リスキリング講座/` use the scoped `business-expansion-reskilling-builder` lecture-first HTML/PDF standard and do not create final `スライド画像/Sxx.png` by default. Do not apply the `isometric-corporate-clean`, dense-slide, fixed-wireframe, chapter-divider, or mandatory TOC-transition rules to that subtree unless the user explicitly asks to move a particular course back into the review-facing image workflow.
...
2. Use `isometric-corporate-clean`. Do not run a multi-template comparison unless the repository rule is changed.
...
- A So What headline that states the conclusion, not only a topic label.
- Course/session context and slide number in a compact header.
...
5. If the user says exercise data looks duplicated, asks for "その回に必要なデータだけ", or asks to fully fix CSV/sample data, read `references/session-specific-exercise-data-workflow.md`. Split or rebuild `演習データ/` by the learner output and demo actually used in each session, then update stale file references.
   - If the user asks to turn exercise data into Google スプレッドシート/ドキュメント, "gws で演習用シートを作って", or "演習データを Drive に上げて", read `references/exercise-data-to-gws-workflow.md` and use `scripts/build_exercise_gws_docs.py`. One session uses one exercise spreadsheet with one tab per CSV; worksheet-type files (`.md`/`.txt`) become Google Docs; all created files go inside that session's Drive `演習データ` folder. Always run `--dry-run` and get user approval before the live run, since this creates Drive files (external send).
...
For image batches, do not use SVG, HTML/CSS, canvas, browser screenshots, local conversion, or overlays. Final slide images must be generated as complete raster images with Codex App Server / GPT image 2 through the `imagegen` skill only.
...
- Generate final visuals with Codex App Server / GPT image 2 through the `imagegen` skill as bitmap images.
...
## Script-After-Slides Ordering

The instructor script is written **after** the slide images exist, not before. The preferred flow is: `スライド案.md` → `画像生成プロンプト.md` → generated `スライド画像/Sxx.png` → read the rendered images → write `講師台本.md`. This way the script reflects what each slide actually shows, instead of guessing at a layout that may change during image generation.

- Before writing the read-aloud text for a slide, open the corresponding `スライド画像/Sxx.png` and note where each element sits: headline, content cards, table, process flow, checklist, right-side or corner isometric scene, and the bottom exercise/output/risk band.
- Write the script so the instructor can verbally point at on-slide positions while recording, e.g. 「右側の図解を見てください」「画面中央の3つのカードのうち、左から…」「下のオレンジの帯に、今日の演習の成果物が書いてあります」. Concrete positional cues (右側/左側/中央/上段/下段/オレンジの帯/青いカード) make recording smooth and reduce reshoots.
- In read-aloud text, do not use production-side labels learners would not naturally say, such as `ヘッドライン`, `内容ブロック`, `中央の本文`, `本文カード`, or `スライド内テキスト`. Translate them into natural video-lecture wording: 「このスライドでは、まず上の大きな一文をご覧ください」「ひとつ目の枠から見ていきます」「右側の図で確認します」「下の確認ポイントを見ます」.
- Assume the instructor records while moving the mouse over the relevant part of the slide. Write phrasing for the learner, not stage directions to the instructor: prefer 「ここは左から右へ順番に見ていきます」 over 「マウスで指してください」. Keep terms gentle but still specific enough that the speaker knows what to point at.
- Keep specialist-term explanations, metaphors, and applied discussion. Do not make the script shorter by removing terms; instead introduce them naturally, e.g. 「ここで、言葉の確認をしておきます」, then explain with an SME-friendly metaphor and an applied business note.
- Avoid making every slide end with the same mechanical sentence. Rotate closing patterns so the script sounds like a human video lecture: connect to the next work, ask learners to map it to their own job, or remind them which output/review point this slide supports.
- Only reference elements that are genuinely on the rendered slide. If the script mentions 「右側の図解」 but the image has no right-side diagram, fix the script or regenerate the slide so the two agree.
- If slides are regenerated or their layout changes, re-check the affected script blocks so positional references still match.
- When an early script draft is needed before images exist (for pacing or review), mark positional references as provisional and finalize them only after the slide images are in place.
...
2. **On-slide positional references**: after reading each `スライド画像/Sxx.png`, add cues that point at where things are on the slide (「右側の図解」「中央のカード」「下の帯」) so the instructor can present without hunting for elements while recording
3. **Metaphor addition**: add 1–2 SME-appropriate metaphors to abstract explanations
4. **Industry-specific examples**: replace generic examples ("問い合わせ管理") with sector-specific ones
5. **Natural phrasing**: rewrite read-aloud text so the instructor can say it comfortably in one breath. Remove production labels such as `ヘッドライン` and `内容ブロック` from read-aloud text, keep specialist-term explanations, and use learner-facing phrases that match mouse-pointed video lectures.
...
- 講師台本はスライド画像が揃ってから書かれ、各 `スライド画像/Sxx.png` を読み込んだうえで「右側の図解」「中央のカード」「下の帯」などの画面上の位置参照が入っているか
- 台本が参照する画面上の要素（右側の図解など）が、実際の `スライド画像/Sxx.png` に存在しているか（台本と画像が食い違っていないか）
- 読み上げ文に `ヘッドライン`、`内容ブロック`、`中央の本文`、`本文カード`、`指してください` のような制作側・指示書側の表現が残っていないか。必要に応じて `rg "ヘッドライン|内容ブロック|中央の本文|本文カード|指してください" 講師台本.md` で確認する。
- 専門用語の補足、例え話、応用的な実務説明を削らず、「ここで、言葉の確認をしておきます」のように自然な講義表現へ整えているか。
