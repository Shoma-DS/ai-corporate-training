...
- ブラウザ操作担当: `GPT-5.3 Codex Spark` 相当の低コストモデルで、Kimi WebBridgeのクリック、待機、スクリーンショット、ページ移動だけを担当する。
- 画像再生成担当: Magic Layersでは直せず、元スライド画像の再生成が必要な場合だけ、GPT image 2を使える高精度・画像対応モデルで再生成する。

モデル指定やサブエージェント呼び出しができない環境では、同じ役割を手順として分け、スクリーンショット回数、`snapshot` 回数、ページ移動回数を抑える。
